...
class TodoSchema(Schema):
    ...
    posted_on = fields.DateTime(dump_onl=True)
...
def requires_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not auth or not check_auth(auth.username, auth.password):
            ...
            resp.headers["WWW-Authenticate"] = 'Base realm="Example"'
            return resp
...
def register():
    ...
    try: # Use get to see if user already exists
        ...
        message = f"Sucessfully created user: {user.email}"
    else:
